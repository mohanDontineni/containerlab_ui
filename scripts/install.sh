...
upgrade(){
  if [ -n "$values" ];then helm upgrade --install "$release" "$chart" --namespace "$namespace" --create-namespace -f "$values" --set "image.repository=$image_repository" --set "image.tag=$image_tag" --set "storageClass=$storage_class" $adopt_args --wait --timeout 10m;
  else helm upgrade --install "$release" "$chart" --namespace "$namespace" --create-namespace --set "image.repository=$image_repository" --set "image.tag=$image_tag" --set "storageClass=$storage_class" $adopt_args --wait --timeout 10m;fi
}
