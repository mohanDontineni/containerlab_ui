...
const forceRepair=process.env.FORCE_REGISTRY_REPAIR==="true";
...
try{
  ...
  const priorBuilds=new Set((evidence?.builds||[]).map(row=>row.job_identity));
  const scheduled=forceRepair||!evidence?.publications?.some(row=>row.compatibility?.registry_mirror?.verified);
  if(scheduled)await repair.click();
  ...
  while(Date.now()<deadline){
    ...
    const build=evidence?.builds?.find(row=>row.recipe_version==="node-containerd-registry-v2"&&(!scheduled||!priorBuilds.has(row.job_identity)));
