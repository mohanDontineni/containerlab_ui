...
async function closeConsole(deviceId:string){const pane=panes.get(deviceId);if(!pane)return;const socket=pane.socket;pane.socket=null;panes.delete(deviceId);if(activeId===deviceId)activeId=[...panes.keys()].at(-1)||'';render();socket?.close();await revoke(pane);pane.terminal.dispose()}
async function reconnect(deviceId:string){const pane=panes.get(deviceId);if(!pane)return;const socket=pane.socket;pane.socket=null;socket?.close();await revoke(pane);pane.terminal.clear();pane.terminal.writeln(`\x1b[36mReconnecting to ${pane.device.name}…\x1b[0m`);pane.sessionId='';pane.status='authorizing';render();try{await authorize(pane.device,pane.terminal)}catch(error){pane.status='failed';pane.terminal.writeln(`\x1b[31m${error instanceof Error?error.message:'Reconnect failed'}\x1b[0m`);render()}}
