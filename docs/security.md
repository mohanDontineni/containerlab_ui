...
Writing to the containerd socket is equivalent to node-runtime administration. Publication remains limited to administrator/editor roles and retains checksum re-verification, license acknowledgement, idempotency, and audit events. The chart deploys a namespace-local Distribution registry with a retained filesystem PV, internal ClusterIP, network-policy-restricted ingress, and worker health evidence. Each successful uploaded-image publication retains the selected node's content-addressed runtime tag, pushes the same imported content to the internal registry, and records the manifest only after the registry returns a SHA-256 digest. Configuring and verifying launcher pulls from that registry on every Kubernetes worker remains required before claiming multi-node distribution.

Platform data-plane ingress is workload-scoped with Kubernetes NetworkPolicy. The TLS gateway is the only selected source for web and console services. PostgreSQL accepts only web, worker, scheduler, console, and migration pods; Redis accepts only the long-running application workloads; the OCI registry accepts the worker and labeled image-publication jobs. A worker probe validates every target selector, allowed source set, and TCP port before publishing the expiring dashboard health signal. Egress is not default-denied because Clabernetes reconciliation and image publication require the Kubernetes API and external registries; installations that add egress isolation must explicitly model those destinations and DNS.
