...
| Namespace-local OCI registry service | Supported | Distribution 3.1.1 runs behind an internal ClusterIP with retained local storage, restricted same-namespace ingress, Kubernetes probes, and expiring worker health evidence. |
| Uploaded-image registry mirroring | Supported | The publication job checksum-verifies and imports the archive, retains the node-local runtime reference, pushes its manifest and layers to the internal registry, then verifies and records the registry's returned manifest digest. |
| Multi-node launcher pull from internal registry | Unverified | Node trust, registry routing, and Clabernetes launcher resolution must be verified on every worker before this is claimed. |
