...
## 65 — Template-to-image compatibility

![Template-to-image compatibility](65-template-image-compatibility.png)

Every immutable template version now carries explicit image requirements for content-addressed identity, architecture, category, format, and optional verified-publication status. The device-template page evaluates up to 100 publications from projects visible to the operator and classifies each as Compatible, Warning, or Incompatible with exact reasons. A ready local publication is distinct from an unverified external registry reference; the latter remains selectable only when policy permits and retains its runtime-pull warning.

The topology editor receives the same server-owned decision for every active template, disables incompatible choices, and explains the selected image's evidence. Draft persistence and Clabernetes runtime preflight independently re-evaluate pinned template/image pairs, so a crafted request cannot bypass the GUI. Firefox found three compatible FRR publications and one warning in the live matrix, then opened a saved BGP draft and verified `FRRouting 10.4.1 · amd64 · compatible` with its immutable-validation explanation. Exact release `0ef05b4` ran on web, worker, scheduler, and console; production routers `r1` and `r2` remained Running/Ready.

## 66 — Explicit deployment plan

![Explicit deployment plan](66-explicit-deployment-plan.png)

The topology Deploy action now opens a server-generated plan instead of a browser confirmation. It identifies the exact saved draft, device and link counts, current and post-action runtime capacity, immutable publication, the new isolated Kubernetes namespace, and every same-lab runtime that will remain pinned and unchanged. When active runtimes exist, the operator must explicitly acknowledge that the action creates an additional environment; the API rejects missing acknowledgement, stale draft identifiers, and unsupported strategies before mutating a revision.

Firefox opened the saved Firewall Policy Reference draft with one active revision-1 runtime. The production project was already at its enforced 5/5 runtime limit, so the plan visibly reported `6 / 5`, explained the quota blocker, required the pinned-runtime acknowledgement, and kept the publish action disabled even after acknowledgement. The verifier closed the dialog without mutation. Exact release `98ceba9` ran on web, worker, scheduler, and console; production BGP routers `r1` and `r2` remained Running/Ready.

## 67 — Server topology preflight

![Server topology preflight](67-server-topology-preflight.png)

The topology toolbar's Validate control now executes a read-only server-authoritative deployment preflight instead of acting as a passive client-side counter. The report binds its evidence to the saved revision ID, edit version, and checksum, then applies the same pinned Clabernetes adapter validation used by deployment. Six platform checks cover saved-draft identity, topology structure, immutable image compatibility, startup-configuration policy, interface requirements, and the version-locked `c9s.run/v1alpha1` compiler.

Each device row explains its immutable image and architecture, linked/free/required interface counts, configuration state, template version and kind, and enforced CPU/memory policy. Blocking findings and non-blocking template or runtime advisories remain distinct, while configuration content, credentials, and artifact storage paths are never returned. Firefox validated the real `BGP Save As Live fa23e34` draft with two FRR routers, one point-to-point link, two encrypted startup configurations, pinned compatible FRR images, and all 6/6 checks passed. The verifier closed the report without mutation. Exact release `206b86e` ran on web, worker, scheduler, and console; production routers `r1` and `r2` remained Running/Ready.

## 68 — Containerlab interoperability

![Containerlab interoperability](68-containerlab-interoperability.png)

The optional Interop workspace connects Studio's bounded Containerlab parser and topology exporter to a guarded GUI workflow. Import accepts at most 2 MiB and 250 devices, rejects unknown or host-escape fields such as binds, commands, capabilities, and host networking, validates point-to-point endpoint reuse, and presents structural incompatibilities before any mutation. Every source node must be explicitly mapped to an active versioned Studio template whose interface inventory covers the imported links and to a project-authorized immutable image that passes that template's compatibility policy.

Import confirmation binds the uploaded topology checksum and expected current draft, requires an idempotency key and active edit lease, transactionally replaces only the editable draft, and preserves published revisions and runtime namespaces. External `startup-config` paths are never read from the client; the operator must acknowledge their omission and add configuration through Studio's encrypted inspector. The `.clab.yml` export contains the portable visual node/image/link graph; the native Studio backup remains the full-fidelity format for encrypted startup configuration, annotations, and editor metadata.

Firefox exported the real two-router `BGP Save As Live fa23e34` visual graph as `bgp-save-as-live-fa23e34.clab.yml`, verified two nodes and one link, uploaded the exact download, selected `FRR Router v1 · verified` for both nodes, and confirmed both immutable `FRRouting 10.4.1 · amd64` publications were recognized as source matches. The mapped import action became available, then the verifier cancelled without mutation. Exact release `a8ba8d6` ran on web, worker, scheduler, and console; production routers `r1` and `r2` remained Running/Ready.

## 83 — Network-health evidence export

![Network-health evidence export](83-network-health-evidence-export.png)

The whole-topology diagnostic panel now exports the latest successful traffic snapshot and reachability matrix without requiring YAML or appliance commands. The ZIP contains normalized `traffic.csv` and `reachability.csv` evidence plus `manifest.json`, which binds each member's byte size and SHA-256 checksum to the deployment, immutable revision, source operation identifiers, completion times, row counts, and export time. Spreadsheet-active prefixes in text fields are neutralized, raw command output is excluded, empty exports are rejected, and each download creates a content-free project audit event with the archive checksum.

Firefox restored the production BGP observations, enabled the export only after evidence was available, and downloaded `bgp-reference-76c24c1-revision-3-network-health.zip`. The verifier found exactly the three expected members, independently recomputed both member checksums and byte sizes, and confirmed two link-endpoint traffic rows plus two ordered reachability rows. Archive SHA-256 was `a032b9a1241b8e20c1675eb211b9eba0839820b2fed6f12c8226d71fb4898caa`. Exact release `f5e8e8d` ran on web, worker, scheduler, and console while production routers `r1` and `r2` remained Running/Ready.

## 84 — Multi-device split console

![Multi-device split console](84-multi-device-split-console.png)

The embedded runtime console is now a bounded multi-session workspace instead of a single replace-in-place terminal. Ready owned devices open as independent tabs with their own xterm terminal, authenticated WebSocket, 15-minute session, connection state, reconnect action, and close control. Operators can keep one active terminal visible or place the active tab and most recently used peer side by side. The server caps each operator at eight active sessions per runtime; viewers remain read-only; and closing or reconnecting explicitly revokes the prior server session before authorization is replaced.

Firefox opened the real production `r1` and `r2` FRR appliances, received two independent connected sessions, selected Split, and ran the harmless `hostname` command in each actual appliance shell. The left pane returned `r1` and the right pane returned `r2`. After the screenshot, Firefox closed both tabs and observed two successful server-side `204` revocations. The acceptance run first exposed a close/rerender race; release `305304d` contains and verifies the corrected immediate-removal ordering. Web, worker, scheduler, and console ran that exact release while both routers remained Running/Ready.

## 85 — Topology jobs and events

![Topology jobs and events](85-topology-jobs-events-panel.png)
